...
export default function AnimeCard({
  ...
  // When pipe is down, counts come back as 0/0. Fall back to AniList episode data so
  // cards still show something useful (aired count for ongoing, total for finished).
  const pipeHasData = (subCount ?? 0) > 0 || (dubCount ?? 0) > 0;
  const anilistAired = (() => {
    if (pipeHasData) return null;
    if (anime.nextAiringEpisode?.episode) return anime.nextAiringEpisode.episode - 1;
    if (
      (anime.status === "FINISHED" || anime.status === "CANCELLED") &&
      anime.episodes != null
    )
      return anime.episodes;
    return null;
  })();
  const sub = pipeHasData ? (subCount ?? 0) : (anilistAired ?? subCount ?? 0);
    ...
    <Link href={`/anime/${anime.id}`} className={cn("group block", className)}>
      ...
      <h3
        className={cn(
          ...
          compact ? "mt-1 text-[10px]" : medium ? "mt-1.5 text-[11px]" : "mt-2 text-sm"
        )}
      >
        {title}
      </h3>
