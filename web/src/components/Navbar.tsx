...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[var(--border)] bg-[var(--background)]/95 backdrop-blur-sm">
      <div className="mx-auto flex h-[52px] max-w-5xl items-center gap-4 px-4">
        ...
        <div ref={ref} className="relative hidden flex-1 sm:block">
          ...
          {open && suggestions.length > 0 && (
            <div className="absolute top-full z-50 mt-1 w-full overflow-hidden rounded-md border border-[var(--border)] bg-[var(--card)] py-1 shadow-lg">
              {suggestions.map((s) => (
                <button
                  key={s.id}
                  type="button"
                ...
                >
                  {s.poster ? (
                    <img
                      src={s.poster}
                      alt=""
                      className="h-12 w-8 shrink-0 rounded object-cover bg-[#1a1d24]"
                      loading="lazy"
                    />
                  ) : (
        ...
        <nav className="ml-auto flex items-center gap-3 text-sm">
          <Link href="/browse" className="hidden text-[var(--muted)] hover:text-[var(--foreground)] sm:inline">
            Browse
          </Link>
          <Link
