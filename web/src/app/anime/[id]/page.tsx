...
export default function AnimeOverviewPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Always fetch rich relations (the main query returns minimal node data: no
      // bannerImage, startDate, isAdult — which breaks ordering and season card images).
      ...
      if (baseSeasons.length) {
        // Fetch one more hop so the full franchise is visible regardless of which season you
        // land on (AniList only returns direct neighbours, not transitive ones).
  ...
  const dateLine =
    start && end && start !== end ? `${start} – ${end}` : start || (anime.seasonYear ? String(anime.seasonYear) : null);
  ...
  return (
    <div className="pb-12">
      <section className="relative min-h-[min(72vh,640px)] w-full overflow-hidden">
        <AnimeTrailerBackdrop youtubeId={youtubeId} poster={poster} title={title} />

        <div className="relative z-10 mx-auto flex min-h-[min(72vh,640px)] max-w-6xl flex-col justify-end px-4 pb-10 pt-24">
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-6 flex flex-wrap items-center gap-3">
                <Link
                  href={watchHref}
                  className="inline-flex items-center gap-2 rounded-lg bg-[var(--accent)] px-6 py-3 font-mono text-sm font-bold uppercase tracking-wide text-black shadow-lg shadow-[var(--accent)]/25 transition-transform hover:scale-[1.02]"
                >
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-2 rounded-lg border px-4 py-3 font-mono text-xs font-semibold backdrop-blur-md transition-colors ${
                    bookmarked
            ...
            <InfoGrid
              items={[
                ...
                anime.season && anime.seasonYear
                  ? {
                      label: "Season",
                      value: `${formatLabel(anime.season)} ${anime.seasonYear}`,
                    }
                  : null,
