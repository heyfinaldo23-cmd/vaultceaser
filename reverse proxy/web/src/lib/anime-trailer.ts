...
export function resolveTrailer(anime: AnimeMedia): {
  ...
  if (!t) return { youtubeId: null, thumbnail: null };
  // Jikan: trailer is a full URL string
  if (typeof t === "string") {
    const m = t.match(/embed\/([A-Za-z0-9_-]+)/);
    return { youtubeId: m?.[1] ?? null, thumbnail: null };
  }
  // AniList: trailer is {id, site, thumbnail}
  if (t.id && (t.site === "youtube" || !t.site)) {
    return { youtubeId: t.id, thumbnail: t.thumbnail || null };
  }
